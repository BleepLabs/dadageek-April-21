...
AudioInputI2S            i2s2;           //xy=95,219
AudioSynthWaveform       waveform1;      //xy=106,353
AudioMixer4              mixer1;         //xy=243,339
AudioAnalyzePeak         peak1;          //xy=274,193
AudioOutputI2S           i2s1;           //xy=443,332
...
void loop() {
  ...
  if (peak1.available()) {
    pr1 = peak1.read();
    waveform1.frequency((pr1 * 500) + 200);
  }
  ...
  if (current_time - prev_time[1] > 20 && 1) { //cahnge to && 0 to not do this code
    prev_time[1] = current_time;
    Serial.println(pr1 * 100);
  }
