...
AudioSynthWaveform       waveform1;      //xy=278,365
AudioSynthWaveform       waveform2;      //xy=293,407
AudioMixer4              mixer1;         //xy=498,378
AudioOutputI2S           i2s1;           //xy=746,375
AudioConnection          patchCord1(waveform1, 0, mixer1, 0);
AudioConnection          patchCord2(waveform2, 0, mixer1, 1);
AudioConnection          patchCord3(mixer1, 0, i2s1, 0);
AudioConnection          patchCord4(mixer1, 0, i2s1, 1);
AudioControlSGTL5000     sgtl5000_1;     //xy=573,251
...
void loop() {
  ...
  int rate1 = (analogRead(A10) / 1023.0) * 100;

  if (current_time - prev_time[1] > 2 ) { //cahnge to && 0 to not do this code
    prev_time[1] = current_time;
    float pot1 = analogRead(A11);
    int temp1 = map(pot1, 0, 1023, 1, 20);
    int pot2 = map(analogRead(A12), 0, 1023, 1, 8);
    arp_loc++;
    arp_loc %= 6;
    // arp_loc = random(0, 7);

    waveform1.frequency(chromatic[temp1 * pot2]);
