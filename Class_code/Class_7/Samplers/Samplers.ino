...
// must be "uint16_t"
uint16_t SDbank0[50000];

#define input_bank_max_length 44100
// must be "uint16_t"
...
void setup() {
  ...
  //needs to go before the SD code as I'm checking the buttons there
  ...
  //olny load a sample if button 7 and 8 are held down and the sd card is there
  else {
    Serial.println("card initialized.");

    if (digitalRead(36) == 0 && digitalRead(37) == 0) {
      ...
      Serial.print("sd_wav_sample_len ");
      Serial.println(sd_wav_sample_len);
    }
  }
  ...
  sampler1.freqOffset(2); //Just copy paste this line if the sample is from an SD card
...
void loop() {
  ...
  if (current_time - prev_time[1] > 50 && 1) {
    ...
    //  Serial.println(sampler2.current_location());
  }
