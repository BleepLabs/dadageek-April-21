/*
  This code uses peak to analyze a sample and record locations to be used for chopping it up
  The frist sampler play the sample as it is when you play it ir finds the chops and prints them out
  while the other two play the first three chops from it
  ...
  current_location(); //returns the current location in samples from a sampler
  freqOffset(2); //Copy paste this line if the sample is from an SD card   //SD samples work a little differntly that WAV2SKETCH ones. This keeps the SD samples working at the same speed as other ones. really it just dubles the rate you put into frequency
...
void loop() {
  ...
  if (current_time - chop_time > 100) {
    chop_flag = 0; //after 100 millis we can start recording chop locations again
  }
  ...
  if (current_time - prev_time[2] > 5) { //slow all this down a little to reduce jiggly readings
    ...
    //Here the post is made to go from 4.0 when its fully left, 0 when its in the middle and 4.0 again when its on the right
    // but on the left it will play in reverse
    if (raw_freq_pot < 512) { //if the pot is on the left side..
      float temp1 = 4.0 - ((raw_freq_pot / 512.0) * 4.0); //get to pot to 0-1.0, then 0-4.0 then flip it around so it's 4.0 to 0
      ...
      float temp1 = (((raw_freq_pot - 512) / 512.0) * 4.0); //on the right the pot starts at 512 so subract that first
      sampler_freq[0] = temp1;
      sampler1.reverse(0);
      sampler2.reverse(0);
      sampler3.reverse(0);
    ...
    float trimmm = .9; //the chops were a tiny be too long
